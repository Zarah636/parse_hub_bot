...
- **群组视频去重**：同一视频在同一群组内再次发送时，先展示原消息位置并要求确认，不会直接重复刷屏。
...
- 整个群组共享一份重复记录，论坛群组中的不同话题也会相互提示。
- 上次在当前话题发送时不重复显示话题名；只有上次位于其他话题时，才显示该话题名称。
- 原消息链接直接放在正文的“本话题”、话题名或“本群”上，不单独显示查看按钮或链接预览。
- 只有当次解析请求的发起人可以点击“重新分享”或“取消”；确认在 10 分钟后过期。
- 确认后会绕过 Telegram 文件缓存，重新执行解析、下载与上传；`raw` 和 `zip` 模式不参与此项去重。
- 历史记录只在 Telegram 媒体成功发送后写入持久化数据库。升级前的旧消息没有可靠的消息 ID，不会自动回填；升级后首次发送将建立记录。
